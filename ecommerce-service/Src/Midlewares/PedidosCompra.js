...
const validateCompraForIdBody=async(req,res,next)=>{
    ...
    try {
        ...
        //VALIDANDO SI EL PEDIDO FUE ENTREGADO
        if (PedidoEncontrado.dataValues.Estado=='Entregado') return res.json({msg:`El pedido ya fue entregado!`,'Pedido':PedidoEncontrado.dataValues}).end();
...
const validateCompraForIdBodyEstadoPedido=async(req,res,next)=>{
    ...
    try {
        ...
        //EN DADO CASO QUE NO EXISTA UN PEDIDO CON EL ID
        if (!PedidoEncontrado) return res.status(404).json({Problems:`No existe un pedido con el id=${PedidoId}`}).end();
        //AGREGANDO AL AL OBJETO REQUEST EL PEDIDOENCONTRADO
        req.PedidoEncontrado=PedidoEncontrado;
        //SIGUIENDO YA QUE TODO SALIO BIEN
        next();
...
const validateCompraForIdBodyVendedor=async(req,res,next)=>{
    ...
    try {
        ...
        const PedidoEncontrado=await Pedido.findByPk(PedidoId);
        //VALIDANDO SI EL PEDIDO FUE ENTREGADO
        if (PedidoEncontrado.dataValues.Estado=='Entregado') return res.json(`El pedido ya fue entregado!`).end();
        //EN DADO CASO QUE NO EXISTA UN PEDIDO CON EL ID
        if (!PedidoEncontrado) return res.status(404).json({Problems:`No existe un pedido con el id=${PedidoId}`}).end();
...
//EXPORTACIONES
module.exports={validateCompraForIdBody,validateCompraForIdBodyVendedor,validateCompraForIdBodyEstadoPedido};